    ...
    while (!quit) {
        ...
        if (!handlerFun) {
            ...
            if (buf[0] == 'E') {
                ...
                IFLYOS_InterfaceUmbrellaSendAction(evt.c_str(), msg.c_str());
            }
            continue;
        }
...
static int IFLYOS_InterfaceBindEvents(const char *address, const char *events) {
    auto msg = std::to_string(IFLYOS_INTERFACE_CURRENT_VERSION) + " " + address + " " + events;
    return IFLYOS_InterfaceSendEvent(IFLYOS_DO_REGISTER_EVT_RECEIVER, msg.c_str());
}
